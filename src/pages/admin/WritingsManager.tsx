  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => navigate(`${ADMIN_PATH}/writings/edit/new`)} className="flex items-center gap-2 bg-[#1E40AF] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#1E3A8A]"><Plus className="w-4 h-4" /> Add Writing</button>
      </div>
        ...
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {filteredItems.map(item => {
                ...
                <tr key={item._id} className="border-b border-[#334155] last:border-0 hover:bg-[#334155]/20 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right"><div className="flex items-center justify-end gap-2">
                    <button onClick={() => navigate(`${ADMIN_PATH}/writings/edit/${item.id || item._id || ''}`)} className="p-2 text-[#94A3B8] hover:text-[#60A5FA] transition-colors"><Pencil className="w-4 h-4" /></button>
              ...
              {items.length === 0 && <tr><td colSpan={6} className="px-6 py-8 text-center text-[#94A3B8]">No writings yet</td></tr>}
            </tbody>
          </table>
        </div>
      )}
