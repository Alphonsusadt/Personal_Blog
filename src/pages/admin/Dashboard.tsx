...
import { api, getRuntimeCache, setRuntimeCache, ADMIN_PATH } from '../../lib/api';
...
export function Dashboard() {
  ...
  const cards = [
    { label: 'Projects', count: stats.projects, icon: FolderKanban, color: 'bg-blue-500', link: `${ADMIN_PATH}/projects` },
    { label: 'Writings', count: stats.writings, icon: PenLine, color: 'bg-purple-500', link: `${ADMIN_PATH}/writings` },
    { label: 'Books', count: stats.books, icon: BookOpen, color: 'bg-amber-500', link: `${ADMIN_PATH}/books` },
  ];
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-6 hover:border-[#475569] transition-colors">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link to={`${ADMIN_PATH}/writings`} className="text-purple-400 hover:text-purple-300 text-sm font-medium flex items-center gap-1">
              ...
              <div className="space-y-3">
                {recentWritings.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <Link to={`${ADMIN_PATH}/writings/edit/new`} className="text-purple-400 hover:text-purple-300 text-xs">Create your first writing →</Link>
                  </div>
                ) : (
                  recentWritings.map(writing => (
                    <div key={writing._id} className="border-b border-[#334155] last:border-0 pb-3 last:pb-0">
                      <Link to={`${ADMIN_PATH}/writings/edit/${writing.id || writing._id || ''}`} className="block hover:text-purple-400 transition-colors group">
            ...
            <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-6 hover:border-[#475569] transition-colors">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link to={`${ADMIN_PATH}/books`} className="text-amber-400 hover:text-amber-300 text-sm font-medium flex items-center gap-1">
                  View All <Edit className="w-3 h-3" />
                </Link>
              </div>
              <div className="space-y-3">
                {recentBooks.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <Link to={`${ADMIN_PATH}/books/edit/new`} className="text-amber-400 hover:text-amber-300 text-xs">Add your first book review →</Link>
                  </div>
                ) : (
                  recentBooks.map(book => (
                    <div key={book._id} className="border-b border-[#334155] last:border-0 pb-3 last:pb-0">
                      <Link to={`${ADMIN_PATH}/books/edit/${book.id || book._id || ''}`} className="block hover:text-amber-400 transition-colors group">
            ...
            <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-6 hover:border-[#475569] transition-colors">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link to={`${ADMIN_PATH}/projects`} className="text-blue-400 hover:text-blue-300 text-sm font-medium flex items-center gap-1">
                  View All <Edit className="w-3 h-3" />
                </Link>
              </div>
              <div className="space-y-3">
                {recentProjects.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <Link to={`${ADMIN_PATH}/projects/edit/new`} className="text-blue-400 hover:text-blue-300 text-xs">Add your first project →</Link>
                  </div>
                ) : (
                  recentProjects.map(project => (
                    <div key={project._id} className="border-b border-[#334155] last:border-0 pb-3 last:pb-0">
                      <Link to={`${ADMIN_PATH}/projects/edit/${project.id || project._id || ''}`} className="block hover:text-blue-400 transition-colors group">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Link to={`${ADMIN_PATH}/projects/edit/new`} className="flex items-center gap-3 bg-[#1E293B] border border-[#334155] rounded-lg px-5 py-4 hover:border-[#475569] transition-colors">
              <Plus className="w-5 h-5 text-blue-400" />
              <span className="text-sm text-[#F8FAFC]">New Project</span>
            </Link>
            <Link to={`${ADMIN_PATH}/writings/edit/new`} className="flex items-center gap-3 bg-[#1E293B] border border-[#334155] rounded-lg px-5 py-4 hover:border-[#475569] transition-colors">
              <Plus className="w-5 h-5 text-purple-400" />
              <span className="text-sm text-[#F8FAFC]">New Writing</span>
            </Link>
            <Link to={`${ADMIN_PATH}/books/edit/new`} className="flex items-center gap-3 bg-[#1E293B] border border-[#334155] rounded-lg px-5 py-4 hover:border-[#475569] transition-colors">
